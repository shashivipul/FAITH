...
def main():
    ...
    if param['ablation_mode'] == 0: 
        ...
        out_t, _, test_teacher, _, _,node_energy_teacher,best_obs_IF_t,best_ind_IF_t = train_teacher(param, model_t, g, feats, labels, indices, criterion_l, evaluator, optimizer_t)
        ...
        _, test_acc, test_val, test_best, _,best_obs_IF_s,best_ind_IF_s = train_student(param, model_s, g, feats, labels, out_t, indices, criterion_l, criterion_t, evaluator, optimizer_s,node_energy_teacher,KD_model) 
        
        return test_teacher, test_acc, test_val, test_best,best_obs_IF_t,best_ind_IF_t,best_obs_IF_s,best_ind_IF_s
        ...
        model_t = Model(param, model_type='teacher').to(device)
        optimizer_t = optim.Adam(model_t.parameters(), lr=float(1e-2), weight_decay=float(param["weight_decay"]))
        out_t,test_acc, test_val, test_best, state_t,best_obs_IF_t,best_ind_IF_t = train_teacher(param, model_t, g, feats, labels, indices, criterion_l, evaluator, optimizer_t)

        np.savez(output_dir.joinpath("out_teacher"), out_t.detach().cpu().numpy())
        torch.save(state_t, output_dir.joinpath("model_teacher"))

        return test_val, test_acc, test_val, test_best,best_obs_IF_t,best_ind_IF_t
        ...
        model_s = Model(param, model_type='student').to(device)
        optimizer_s = optim.Adam(model_s.parameters(), lr=float(param["learning_rate"]), weight_decay=float(param["weight_decay"]))
        out_s, test_acc, test_val, test_best, state_s,best_obs_IF_s,best_ind_IF_s = train_student(param, model_s, g, feats, labels, out_t, indices, criterion_l, criterion_t, evaluator, optimizer_s, KD_model)  

        np.savez(output_dir.joinpath("out_student"), out_s.detach().cpu().numpy())
        torch.save(state_s, output_dir.joinpath("model_student"))

        return test_teacher, test_acc, test_val, test_best,best_obs_IF_s,best_ind_IF_s
...
if __name__ == "__main__":
    ...
    if param['save_mode'] == 0:
        set_seed(param['seed'])
        test_teacher, test_acc, test_val, test_best,best_obs_IF_t,best_ind_IF_t,best_obs_IF_s,best_ind_IF_s = main()
        ...
        for seed in range(3):
            ...
            obs_IF_list_teacher.append(best_obs_IF_t)
            ind_IF_list_teacher.append(best_ind_IF_t)
            obs_IF_list_student.append(best_obs_IF_s)
            ind_IF_list_student.append(best_ind_IF_s) 
    ...
    print('Teacher obs IF',np.mean(obs_IF_list_teacher),np.std(obs_IF_list_teacher))
    print('Teacher ind IF',np.mean(ind_IF_list_teacher),np.std(ind_IF_list_teacher))
    print('Student Results')
    print('Student accuracy',np.mean(test_val_list)*100, np.std(test_val_list)*100)
    print('Student obs IF',np.mean(obs_IF_list_student),np.std(obs_IF_list_student))
    print('Student ind IF',np.mean(ind_IF_list_student),np.std(ind_IF_list_student))
